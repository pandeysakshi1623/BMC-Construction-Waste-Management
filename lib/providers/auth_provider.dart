...
class AuthProvider extends ChangeNotifier {
  ...
  Future<void> _loadFromPrefs() async {
    ...
    if (token != null && token.isNotEmpty && email != null) {
      _user = UserModel(id: '', email: email, role: role ?? '', token: token);
    }
    _initialized = true;
    notifyListeners();
  }
  ...
  Future<String?> login(String username, String password, {String role = ''}) async {
    ...
    try {
      ...
      if (role == 'bmc') {
        ...
        // Backend now returns 'role' field — use it as the source of truth
        data = await ApiService.login(username, password);
        final backendRole = (data['role'] as String?) ?? '';
        // If backend returned a role, trust it; otherwise fall back to what user selected
